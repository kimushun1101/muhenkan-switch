...
通知は D-Bus 経由 (`org.freedesktop.Notifications`) で送信するため、デスクトップ環境標準の通知デーモン (GNOME Shell / KDE Plasma / mako 等) があれば追加パッケージは不要です。

```bash
sudo apt install xdotool wmctrl xclip x11-utils
```
