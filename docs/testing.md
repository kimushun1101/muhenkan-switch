...
#### 9. タイムスタンプ — V/C/X（Windows 手動テスト）

テキストエディタ（メモ帳等）とエクスプローラーの両方で確認する。

**V（テキスト入力時）: タイムスタンプ入力**

```bash
# テキストエディタにフォーカスした状態で
cargo run -p muhenkan-switch-core -- timestamp --action paste
# または kanata 経由: 無変換+V
```

| 条件 | 期待動作 |
|------|---------|
| テキストエディタにフォーカス | カーソル位置に現在日時（例: `20260313`）が入力される |

**C（テキスト入力時）: プレーンテキストコピー**

```bash
# リッチテキスト（Word, ブラウザ等）を選択した状態で
cargo run -p muhenkan-switch-core -- timestamp --action copy
# または kanata 経由: 無変換+C
```

| 条件 | 期待動作 |
|------|---------|
| リッチテキストを選択した状態 | クリップボードにプレーンテキストとしてコピーされる（書式が除去される） |
| テキスト未選択 | エラー「クリップボードにテキストがありません」（正常系） |

**X（テキスト入力時）: 無効（no-op）**

```bash
cargo run -p muhenkan-switch-core -- timestamp --action cut
# または kanata 経由: 無変換+X
```

| 条件 | 期待動作 |
|------|---------|
| テキストエディタにフォーカス | 何も起こらない |

**V/C/X（エクスプローラー上）: ファイル操作**

| キー | 期待動作 |
|------|---------|
| V | 選択ファイルがファイル更新日時でリネームされる（例: `20260313_file.txt`） |
| C | 選択ファイルがタイムスタンプ付きで複製される |
| X | 選択ファイルのファイル名からタイムスタンプが除去される |
