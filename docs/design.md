...
| タイムスタンプ / プレーンコピー | V: テキスト時は `arboard` でクリップボードをプレーンテキスト化して貼り付け、ファイルマネージャ上ではファイル更新日時でリネーム。C: テキスト時は `chrono` で現在日時を生成し、Windows は `SendInput` で直接入力、Linux はクリップボード経由で貼り付け（IME 全角化を回避）、ファイルマネージャ上ではタイムスタンプ付き複製。X: ファイルマネージャ上でタイムスタンプ除去（テキスト時は no-op） |
...
| 固定アクション | v (ts-paste), c (ts-copy), x (ts-cut) | タイムスタンプ操作（V はテキスト時プレーン貼り付け、C はテキスト時タイムスタンプ入力） |
| 固定アクション | z (position トグル) | タイムスタンプ位置を before/after で切替。Toast でファイル名プレビューを表示 |
...
### CapsLock → Ctrl リマップを kanata で行わない理由

CapsLock → Ctrl のリマップは muhenkan-switch (kanata) の機能としては提供しない。理由は以下の通り:

- **Windows で CapsLock の key-up イベントが正しく伝搬しない**: CapsLock はトグルキーとして OS レベルで特殊処理される。kanata のキーボードフック (`kanata_cmd_allowed`) 経由でリマップすると、key-up が検出されず Ctrl が押されっぱなしになる。AutoHotKey でも同じ問題が発生する。kanata の `defvirtualkeys` + `on-press-fakekey` / `on-release-fakekey` による明示的な press/release 管理でも解決しなかった
- **各 OS に専用のリマップ機構がある**: ドライバレベルやデスクトップ環境レベルで確実に動作する手段が用意されている

#### OS 別の推奨設定

| OS | 方法 |
|----|------|
| Windows | [Ctrl2Cap](https://learn.microsoft.com/sysinternals/downloads/ctrl2cap)（Sysinternals 製、レジストリ Scancode Map を設定するツール）または [PowerToys](https://learn.microsoft.com/windows/powertoys/) の Keyboard Manager |
