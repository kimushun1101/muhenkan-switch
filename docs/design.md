...
| タイムスタンプ / プレーンコピー | V: テキスト時は `chrono` で現在日時を `SendInput` 直接入力、エクスプローラー時はファイル更新日時でリネーム。C: テキスト時は `Ctrl+C` → `arboard` でプレーンテキスト化、エクスプローラー時はタイムスタンプ付き複製。X: エクスプローラー時はタイムスタンプ除去（テキスト時は no-op） |
